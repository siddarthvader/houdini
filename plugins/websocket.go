...
func HandleWebSocketConnection(conn *websocket.Conn) {
	...
	for {
		...
		log.Printf("Received messaes from %s that is %s", conn.RemoteAddr(), msg.Type)
...
func registerWebsocketGenerateHandler(plugin HoudiniPlugin[config.PluginConfig]) {
	...
	wsHandlers["Generate"] = func(conn *websocket.Conn, msg WebSocketMessage) {
		if msg.Type == "request" {
			response := WebSocketResponse{
				ID:    msg.ID,
				Type:  "response",
		...
		ctx := ContextWithTaskID(context.Background(), msg.TaskID)

		result, err := handleGenerate(plugin)(ctx)
		if err != nil {
			response := WebSocketResponse{
				ID:    msg.ID,
				Type:  "response",
